(provide br/inf/ufpr/jessmre/procedures/core/missingsubprocedure/nopartialcolumns/blank_instead_of_borrow)
(require br/inf/ufpr/jessmre/procedures/commons/templates)
(require br/inf/ufpr/jessmre/procedures/commons/functions)

;; Repairs impasse by skipping the column and processing subgoal

; Repara o impasse pulando para o próximo subobjetivo
; Alteração na regra sub1col-borrow.
; Desempilha subobjetivo de emprestar.
...
(defrule sub1Col-borrow
    ...
    (test (< ?t ?b))
	=>
    (modify ?result (result (create$ _ $?r)))
    (modify ?problem (subgoals $?goals))
)
