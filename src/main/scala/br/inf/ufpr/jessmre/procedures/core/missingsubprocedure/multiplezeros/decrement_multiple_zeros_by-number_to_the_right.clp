(provide br/inf/ufpr/jessmre/procedures/core/missingsubprocedure/multiplezeros/decrement_multiple_zero_by_number_to_the_right)

(require br/inf/ufpr/jessmre/procedures/commons/templates)
(require br/inf/ufpr/jessmre/procedures/commons/functions)

(deftemplate decr-amount (slot amount) (slot first))
...
(defrule sub1Col
    ...
    ?decrAmount <- (decr-amount (amount ?amount))    
    (test (>= ?t ?b))
	=>
    (bind ?resp (do-sub ?t ?b))
    (modify ?decrAmount (amount 0)  (first 0))
...
(defrule decr-first
    ...
    ?decrAmount <- (decr-amount (amount ?amount) (first 0))
	(test (> ?t 0))
    =>
    (modify ?subGoal (top (- ?t 1)))
    (modify ?decrAmount (first 1))
    (modify ?problem (subgoals $?goals))
)
...
(defrule decr-others
    ...
    ?decrGoal <- (decr-goal (column ?column))
    ?subGoal <- (sub1col-goal (top ?t) (order ?column))
    ?decrAmount <- (decr-amount (amount ?amount) (first 1))
	(test (> ?t 0))
    =>
    (modify ?subGoal (top (- ?t ?amount)))
    (modify ?decrAmount (amount (- ?amount 1)))
    (modify ?problem (subgoals $?goals))
)
...
(assert (decr-amount (amount 0) (first 0)))
