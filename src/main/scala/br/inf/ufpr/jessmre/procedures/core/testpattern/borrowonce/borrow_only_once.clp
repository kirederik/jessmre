(provide br/inf/ufpr/jessmre/procedures/core/testpattern/borrowonce/borrow_only_once)
...
(defrule decr
    ...
    ?borrow <- (borrow (occur ?occur))
	(test (> ?t 0))
    =>
	(if (eq ?occur 0) then 
        (modify ?subGoal (top (- ?t 1)))
        (modify ?borrow (occur 1))
    )
    (modify ?problem (subgoals $?goals))
)
...
;(assert (subtraction (top 5 3 5) (bottom 2 7 8)))
;(assert (desirable (result 3 5 7)))
