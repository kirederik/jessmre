(provide br/inf/ufpr/jessmre/procedures/core/missingsubprocedure/multiplezeros/decrement_leftmost_zero_only)
(require br/inf/ufpr/jessmre/procedures/commons/templates)
(require br/inf/ufpr/jessmre/procedures/commons/functions)

(deftemplate decr-mult-goal (slot column))
...
(defrule decr-borrow
    ...
    ?decrGoal <- (decr-goal (column ?column))
    ?subGoal <- (sub1col-goal (top ?t) (order ?column))
    ?adjacent <- (adjacent (adj ?toBorrow) (n ?n) (column ?column))
    (test (neq ?toBorrow 0))    
	(test (eq ?t 0))
    =>
    (bind ?borrow (assert (borrow-goal (incr ?column))))
    (modify ?problem (subgoals ?borrow $?goals))
)


(defrule decr-mult-borrow
    "Regra para efetuar o decremento -- Com empréstimo"
    ?problem <- (problem (subgoals ?decrGoal $?goals))
    ?decrGoal <- (decr-goal (column ?column))
    ?subGoal <- (sub1col-goal (top ?t) (order ?column))
	...
	(test (eq ?t 0))
    =>
    (bind ?decrGoal2 (assert (decr-mult-goal (column ?column))))
...
(defrule decr-mult
    "Regra para efetuar o decremento -- Com empréstimo"
    ?problem <- (problem (subgoals ?decrGoal $?goals))
    ?decrGoal <- (decr-mult-goal (column ?column))
    ?subGoal <- (sub1col-goal (top ?t) (order ?column))
    ?adjacent <- (adjacent (adj ?toBorrow) (n ?n) (column ?column))
    (test (eq ?toBorrow 0))
    =>
	(modify ?subGoal (top 10))
    (bind ?multiGoal (assert (decr-mult-goal (column (+ ?column 1)))))
    (modify ?problem (subgoals ?multiGoal $?goals))
)
...
;(assert (subtraction (top 3 0 0 5) (bottom 2 7)))
;(assert (desirable (result 3 9 8 8)))
