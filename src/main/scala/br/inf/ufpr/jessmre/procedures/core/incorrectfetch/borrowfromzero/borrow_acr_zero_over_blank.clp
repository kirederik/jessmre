(provide br/inf/ufpr/jessmre/procedures/core/incorrectfetch/borrowfromzero/borrow_acr_zero_over_blank)
...
(defrule decr-borrow
    ...
    ?decrGoal <- (decr-goal (column ?column))
    ?subGoal <- (sub1col-goal (top ?t) (order ?column))
    (adjacent {column == ?column} {adjbot > -1})    
	(test (eq ?t 0))
    =>
