(provide br/inf/ufpr/jessmre/procedures/core/missingsubprocedure/nopartialcolumns/sub_one_over_blank)
(require br/inf/ufpr/jessmre/procedures/commons/templates)
(require br/inf/ufpr/jessmre/procedures/commons/functions)

;; Repairs impasse by substituting a one for a missing digit

; Repara o impasse substituindo o digito faltante por 1
; Alteração na regra subtract-no-bottom.
; Empilha objetivo de subtrair as colunas sem digito embaixo por 1
...
(defrule subtract-no-bottom
    ...
    =>
    (assert (adjacent (n (first$ $?ttail)) (adj ?thead) (column (- (length$ $?ttail) 1))))
    (bind ?sub1ColGoal (assert (sub1col-goal (top ?thead) (bottom 1) (order (length$ $?ttail)))))
