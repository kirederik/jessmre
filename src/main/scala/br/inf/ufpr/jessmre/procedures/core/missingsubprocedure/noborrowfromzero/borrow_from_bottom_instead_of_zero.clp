(provide br/inf/ufpr/jessmre/procedures/core/missingsubprocedure/noborrowfromzero/borrow_from_bottom_instead_of_zero)
(require br/inf/ufpr/jessmre/procedures/commons/templates)
(require br/inf/ufpr/jessmre/procedures/commons/functions)


;; Reinterprets the location of the decrement as any non-zero digit in the adjacent column

; Caso tenha que decrementar 0, escolhe qualquer número diferente de 0 da coluna adjacente.
; Alteração na regra decr-borrow.
; Decrementa de baixo caso top == 0

(deftemplate bottomadjacent (slot adj) (slot n) (slot column))
(deftemplate decr-bottom-goal (slot column))
...
(defrule subtract 
    ...
    (assert (bottomadjacent (n (first$ $?btail)) (adj ?bhead) (column (- (length$ $?btail) 1))))
...
;; BUGGY RULE
(defrule decr-bottom
    "Regra para efetuar o decremento embaixo -- Sem empréstimo"
    ?problem <- (problem (subgoals ?decrGoal $?goals))
    ?decrGoal <- (decr-bottom-goal (column ?column))
    ?subGoal <- (sub1col-goal (bottom ?b) (order ?column))
    =>
    (modify ?subGoal (bottom (- ?b 1)))
    (modify ?problem (subgoals $?goals))
)
...
(defrule decr-borrow
	...
	(test (eq ?t 0))
    =>
    (bind ?decr (assert (decr-bottom-goal (column ?column))))
...
;(assert (subtraction (top 3 0 5) (bottom 2 7)))
;(assert (desirable (result 2 9 8)))
