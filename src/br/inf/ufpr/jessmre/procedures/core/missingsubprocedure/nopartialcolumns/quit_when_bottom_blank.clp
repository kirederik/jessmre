(provide br/inf/ufpr/jessmre/procedures/core/missingsubprocedure/nopartialcolumns/quit_when_bottom_blank)

(require br/inf/ufpr/jessmre/procedures/commons/templates)
(require br/inf/ufpr/jessmre/procedures/commons/functions)

;; Repairs impasse by going on to the next column or the next problem

; Repara o impasse quando não há dígito na parte de baixo ignorando a parte de cima
; Alteração na regra subtract-no-bottom.
; Não empilha objetivo de subtrair coluna que tem vazio embaixo.
; Importante notar que, apesar de não empilhar, insere-se o sub1col na base de fatos, para realizar o empréstimo corretamente.
...
(defrule subtract-no-bottom
    ...
    =>
    (assert (adjacent (n (first$ $?ttail)) (adj ?thead) (column (- (length$ $?ttail) 1))))
    (assert (sub1col-goal (top ?thead) (bottom 0) (order (length$ $?ttail))))
    (bind ?subNextGoal (assert (subtract-goal (top ?ttail) (bottom (create$ ?bhead ?btail)))))
    (bind ?goals (create$ $?endGoal))
	(modify ?problem (subgoals ?subNextGoal ?goals))
)
...
(assert (subtraction (top 9 6 0) (bottom 4)))
(assert (desirable (result 6)))
