(provide br/inf/ufpr/jessmre/procedures/core/misc/n-n_after_borrow_causes_borrow)

(require br/inf/ufpr/jessmre/procedures/commons/templates)
(require br/inf/ufpr/jessmre/procedures/commons/functions)

(deftemplate original (slot init) (slot column))
; Initial rule
...
(defrule subtract 
	...
	(test (> (length$ $?ttail) 0))
    =>
    (assert (original (init ?thead) (column (length$ ?ttail))))
...
(defrule sub1Col-borrow
    ...
    ?subGoal <- (sub1col-goal (top ?t) (bottom ?b) (order ?order))
    ?result <- (subtraction (result $?r))
    ?original <- (original (init ?init) (column ?order))
    (test (<= ?t ?b))
	=>
    (if (and (eq ?t ?init) (eq ?t ?b)) then 
        (bind ?resp (do-sub ?t ?b))
    	(modify ?result (result (create$ ?resp $?r)))
    	(modify ?problem (subgoals $?goals))
    else 
...
(assert (subtraction (top 9 5 3 3) (bottom 1 4 7 3)))
(assert (desirable (result 7 10 6 0)))
