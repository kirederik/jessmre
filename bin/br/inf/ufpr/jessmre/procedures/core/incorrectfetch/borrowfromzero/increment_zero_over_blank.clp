(provide br/inf/ufpr/jessmre/procedures/core/incorrectfetch/borrowfromzero/increment_zero_over_blank)

(require br/inf/ufpr/jessmre/procedures/commons/templates)
(require br/inf/ufpr/jessmre/procedures/commons/functions)

(deftemplate incr-goal (slot column))
...
(defrule borrow
    ...
    ?adjacent <- (adjacent (adj ?toBorrow) (adjbot ?adjbot) (column ?column))
    =>
    ;(printout t "We have a " ?t " at column " ?column " that will borrow from " ?toBorrow " at column " (+ ?column 1) crlf)    
    (modify ?subGoal (top (+ ?t 10)))
    
    (if (and (eq ?toBorrow 0) (eq ?adjbot -1)) then
       (bind ?incrGoal (assert (incr-goal (column (+ ?column 1))))) 
       (modify ?problem (subgoals ?incrGoal $?goals))
    else    
...
(defrule incr
    ...
    ?problem <- (problem (subgoals ?incrGoal $?goals))
    ?incrGoal <- (incr-goal (column ?column))
...
(defrule decr-borrow
    ...
    ?problem <- (problem (subgoals ?decrGoal $?goals))
    ?decrGoal <- (decr-goal (column ?column))
    ?subGoal <- (sub1col-goal {top == 0} (order ?column))  
    =>
...
(assert (subtraction (top 4 0 2) (bottom 6)))
(assert (desirable (result 4 1 6)))
